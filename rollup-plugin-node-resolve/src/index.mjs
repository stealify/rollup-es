...
import { existsSync, realpathSync, stat, readFile } from 'fs';
...
const ES6_BROWSER_EMPTY = resolve( './empty' );
...
function cachedReadFile (file, cb) {
	if (file in readFileCache === false) {
		readFileCache[file] = promisify(readFile)(file).catch(err => {
			delete readFileCache[file];
			throw err;
		});
	}
	//console.log('WILL READ: %s', file)
...
function cachedIsFile (file, cb) {
	if (file in isFileCache === false) {
		isFileCache[file] = promisify(stat)(file)
			.then(
				stat => stat.isFile(),
				err => {
				...
				packageFilter ( pkg, pkgPath ) {
					...
					if (options.browser && typeof pkg[ 'browser' ] === 'object') {
						packageBrowserField = Object.keys(pkg[ 'browser' ]).reduce((browser, key) => {
							...
							if (pkgRoot.indexOf('can-kefir') > -1) {
								console.log(key,browser,pkgRoot, pkg[ 'browser' ][ key ])
								return
							}
			...
			return resolveIdAsync(
				...
				.then(resolved => {
					...
					if ( !disregardResult && resolved !== false ) {
						if ( !preserveSymlinks && resolved && existsSync( resolved ) ) {
					...
					if ( resolved && options.modulesOnly ) {
						return promisify(readFile)( resolved, 'utf-8').then(code => isModule( code ) ? resolved : null);
					} else {
